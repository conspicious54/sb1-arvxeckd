  ...
  if (loading) {
    return (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-green-50 dark:border-gray-700 p-4 md:p-6 animate-pulse">
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
      {statItems.map((stat) => (
        <div
          key={stat.label}
          className="relative bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-green-50 dark:border-gray-700 p-4 md:p-6 hover:shadow-xl transition-all duration-200 group overflow-hidden"
        >
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-2 md:mb-4">
              <div className={`p-2 md:p-2.5 bg-${stat.color}-50 dark:bg-${stat.color}-900/30 rounded-lg`}>
                {stat.icon}
              </div>
            </div>
            <h4 className="text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400 mb-0.5 md:mb-1">{stat.label}</h4>
